...
use App\HomeSlide;
use App\Http\Resources\HomeSlide as HomeSlideResource;
...
class HomeSlideController extends Controller
{
    public function index()
    {
        return response()->json([
            ...
            'images' => HomeSlide::pluck('large_image')->all()
        ], 200);
    }
    ...
    public function store(Request $request)
    {
       // if ($request->large_image) {
             $name = str_random().'.'.explode('/', explode(':', substr($request->large_image, 0, strpos($request->large_image, ';')))[1])[1];
