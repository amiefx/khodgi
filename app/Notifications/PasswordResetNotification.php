...
class PasswordResetNotification extends Notification
{
    ...
    {
        $urlToResetForm = "http://localhost:8000/resetpassword/". $this->token .'?email='.urlencode($notifiable->email);
        return (new MailMessage)
           ...
           // ->action(Lang::get('Reset Password'), url(config('app.url').'/resetpassword/'.'?token='.$this->token).'?email='.urlencode($notifiable->email))
